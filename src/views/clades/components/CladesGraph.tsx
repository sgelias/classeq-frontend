...
export default () => {
    ...
    const graphData = () => {
        ...
        const treeData = clades.map((item, index) => ({
            uuid: item.uuid,
            parent: item.parent,
            name: item.name ? item.name : `Node_${index}`,
            value: clades.filter(clade => clade.parent === item.uuid).length
        }));

        const data0 = arrayToTree(treeData, {
        ...
        Sunburst()
            .data(data0[0])
            .color((item) => {
                console.log(item.name, item.__dataNode);
                let parentId = item.__dataNode?.parent?.id;
                return parentId ? colors[parentId + 2] : "grey";
            })
            //.onClick(el => onGraphClick(el))
            
            /* .tooltipTitle((item) => {
                return item.__dataNode ? colors[item.__dataNode?.depth] : 'grey';
            }) */
